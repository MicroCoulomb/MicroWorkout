...
export async function synchronize(userId: string, request: SyncRequest) {
  ...
  for (const mutation of request.mutations) {
    await db.transaction(async (tx) => {
      ...
      if (mutation.operation === "delete") await applyDelete(tx, userId, mutation.entityType, mutation.entityId);
      ...
      await tx.insert(syncChanges).values({ userId, entityType: mutation.entityType, entityId: mutation.entityId, operation: mutation.operation, payload: mutation.operation === "upsert" ? mutation.payload : null });
...
async function applyUpsert(tx: Transaction, userId: string, entityType: SyncRequest["mutations"][number]["entityType"], payload: unknown) {
  ...
  if (entityType === "profile") {
    ...
    await tx.insert(userProfiles).values({ userId, weightUnit: profile.weightUnit, weeklyGoal: profile.weeklyGoal, timezone: profile.timezone }).onConflictDoUpdate({ target: userProfiles.userId, set: { weightUnit: profile.weightUnit, weeklyGoal: profile.weeklyGoal, timezone: profile.timezone, updatedAt: new Date() } });
    return;
  }
  if (entityType === "plan") {
...
async function applyDelete(tx: Transaction, userId: string, entityType: SyncRequest["mutations"][number]["entityType"], entityId: string) {
  if (entityType === "plan") { await tx.update(workoutPlans).set({ deletedAt: new Date(), updatedAt: new Date() }).where(and(eq(workoutPlans.id, entityId), eq(workoutPlans.userId, userId))); return; }
  ...
  if (entityType === "session") {
    const [session] = await tx.select({ status: workoutSessions.status }).from(workoutSessions).where(and(eq(workoutSessions.id, entityId), eq(workoutSessions.userId, userId))).limit(1);
