...
export function SettingsScreen({ isAdmin, canDeleteAccount, syncEnabled, onAdmin }: { isAdmin: boolean; canDeleteAccount: boolean; syncEnabled: boolean; onAdmin(): void }) {
  ...
  return <>
    ...
    <div className="settings-grid">
      ...
      <section className="danger-zone card"><span className="eyebrow">Account controls</span><h2 className="display">Private data</h2><p>Clear cached records on this device, sign out, or permanently delete your account and owned data.</p><button className="button-secondary" onClick={() => setPendingAction("clear")}><Trash2 size={18} /> Clear this device</button>{syncEnabled && <button className="button-secondary" onClick={async () => { await store.syncNow(); window.localStorage.removeItem("microworkout-last-user"); await authClient.signOut(); router.refresh(); }}><LogOut size={18} /> Sign out</button>}{syncEnabled && canDeleteAccount && <button className="button-danger" onClick={() => setPendingAction("delete-account")}><Trash2 size={18} /> Delete account</button>}</section>
